 ...
 *
 * ✅ Layout-Änderung (DEIN WUNSCH):
 * - Vertikal statt 2 Spalten:
 *   - oben: 1/3 Bild/Preview
 *   - unten: 2/3 Konfigurator
 * - Unterer Bereich scrollt bei wenig Höhe
 *
 * ✅ Fix (dieser Patch):
...
// ✅ STACKED LAYOUT: Preview ist nur oben (≈ 1/3), daher VH kleiner
const PREVIEW_MAX_VH_FACTOR = 0.30;
...
function normalizeUrl(u) {
  ...
  if (s.startsWith("//")) {
    const proto =
      typeof window !== "undefined" && window.location && window.location.protocol ? window.location.protocol : "https:";
    return `${proto}${s}`;
  }
  return s;
}
...
function buildFreeformMasterMask({ imgEl, imgAspect, getMasterRectFromAspect, maxMaskDim = 520, alphaThreshold = 8, padPx = 120 }) {
...
export default function StickerCanvasClient({
  ...
  const previewDims = useMemo(() => {
    ...
    const maxH = Math.max(200, vp.h * PREVIEW_MAX_VH_FACTOR);
  ...
  // ==============================
  // Render (STACKED: oben Preview, unten Controls)
  // ==============================
  return (
    <div style={styles.wrapper}>
      {/* TOP: Preview (1/3) */}
      <div style={{ ...styles.topPanel, background: previewBg }}>
        {!imageUrl ? (
          <div style={styles.emptyUploadWrap}>
            <div style={styles.emptyHint}>Bitte unten ein Bild hochladen.</div>
      ...
      <div style={styles.bottomPanel}>
        ...
        {/* Upload unten: wenn Bild existiert -> "Bild ändern", sonst CTA */}
        {imageUrl ? (
          <button type="button" style={styles.secondaryBtn} onClick={openFilePicker}>
            {uploading ? "Upload…" : "Bild ändern"}
          </button>
        ) : (
          <button type="button" style={styles.secondaryBtn} onClick={openFilePicker} disabled={uploading}>
            {uploading ? "Upload…" : "Bild hochladen"}
          </button>
        )}
...
const styles = {
  // ✅ STACKED WRAPPER: oben 1/3, unten 2/3
  wrapper: {
    width: "100%",
    maxWidth: 980,
    margin: "0 auto",
    ...
    gridTemplateRows: "1fr 2fr", // ✅ 1/3 Preview, 2/3 Config
    gap: 0,
    borderRadius: 22,
    overflow: "hidden",
    ...
    // sorgt dafür, dass das Verhältnis auch sichtbar wird (und unten scrollbar)
  ...
  bottomPanel: {
    ...
    background: "#0b0f16",
    color: "#fff",
    overflowY: "auto", // ✅ wenn wenig Höhe: Konfigurator scrollt
